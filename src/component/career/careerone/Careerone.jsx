...
import { FiClock, FiMapPin, FiBriefcase, FiGlobe, FiAlertCircle, FiCheckCircle } from "react-icons/fi";
...
const VALIDATORS = {
  name: {
    ...
    filter:   v => v.replace(/[^a-zA-Z\s'-]/g, ""),   // strip digits & special chars live
  ...
  phone: {
    ...
    filter:   v => v.replace(/\D/g, "").slice(0, 10),  // digits only, max 10
    error:    "Phone number must be exactly 10 digits (no letters or spaces).",
  },
  cover: {
...
const Blob = ({ color, w, h, top, left, delay = 0 }) => (
  <motion.div
    style={{ position: "absolute", width: w, height: h, top, left, background: color, filter: "blur(70px)", opacity: .15, borderRadius: "60% 40% 30% 70%/60% 30% 70% 40%", pointerEvents: "none", zIndex: 0 }}
...
───────────────────────────────────────── */
const ValidatedField = ({ label, fieldKey, value, onChange, touched, errors, placeholder, type = "text", readOnly = false, style = {} }) => {
  const hasError  = touched[fieldKey] && errors[fieldKey];
  const isValid   = touched[fieldKey] && !errors[fieldKey] && value.trim().length > 0;

  return (
    <div>
      <label style={{
        fontSize: 11, color: hasError ? "#f87171" : isValid ? "#4ade80" : "#64748b",
        ...
        {label}
        {hasError && <span style={{ marginLeft: 4 }}>*</span>}
      </label>
      ...
      <div style={{ position: "relative" }}>
        <input
          ...
          className="cp-input"
          style={{
            paddingRight: (isValid || hasError) ? 40 : 16,
        ...
        {/* Status icon */}
        {hasError && (
          <div style={{ position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)", color: "#f87171", display: "flex" }}>
            <FiAlertCircle size={16} />
          </div>
        )}
        {isValid && (
          <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", stiffness: 300 }}
            style={{ position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)", color: "#4ade80", display: "flex" }}>
      ...
      <AnimatePresence>
        {hasError && (
          ...
          >
            <FiAlertCircle size={11} style={{ marginTop: 1, flexShrink: 0 }} />
            {errors[fieldKey]}
          </motion.p>
        )}
      </AnimatePresence>
      ...
      {/* Phone helper */}
      {fieldKey === "phone" && !hasError && (
        <p style={{ fontSize: 10, color: "#334155", marginTop: 5, fontFamily: "'Space Mono',monospace" }}>
          Digits only · exactly 10 numbers · {(10 - value.length)} remaining
        </p>
      )}
      {/* Name helper */}
      {fieldKey === "name" && !hasError && touched[fieldKey] && (
        <p style={{ fontSize: 10, color: "#334155", marginTop: 5, fontFamily: "'Space Mono',monospace" }}>
...
const Careerone = () => {
  ...
  const validateField = (key, value) => {
    ...
    if (!v) return "";
    if (!value || value.trim() === "") return `${key.charAt(0).toUpperCase() + key.slice(1)} is required.`;
    return v.validate(value) ? "" : v.error;
  };

  /* ── Handle change with live filtering + validation ── */
  const handleChange = (key, rawValue) => {
    // Apply character filter (e.g. strip digits from name, strip letters from phone)
    const filtered = VALIDATORS[key]?.filter ? VALIDATORS[key].filter(rawValue) : rawValue;
    ...
    const err = validateField(key, filtered);
  ...
  const validateAll = () => {
    const fields = ["name", "email", "phone", "cover"];
    const newErrors = {};
    const newTouched = {};
    let valid = true;
    fields.forEach(k => {
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .cp-glass { background:rgba(255,255,255,0.04); backdrop-filter:blur(20px); -webkit-backdrop-filter:blur(20px); border:1px solid rgba(255,255,255,0.08); }
        .cp-glass-md { background:rgba(255,255,255,0.06); backdrop-filter:blur(28px); -webkit-backdrop-filter:blur(28px); border:1px solid rgba(255,255,255,0.1); }
        .cp-grid { background-image:linear-gradient(rgba(148,163,184,0.025) 1px,transparent 1px),linear-gradient(90deg,rgba(148,163,184,0.025) 1px,transparent 1px); background-size:72px 72px; }

        .cp-btn-primary {
          background:linear-gradient(135deg,#0ea5e9,#6366f1); border:none; cursor:pointer; color:#fff;
          font-weight:600; font-family:'Outfit',sans-serif; font-size:14px;
          padding:13px 28px; border-radius:12px; transition:all .3s ease;
        ...
        .cp-btn-ghost {
          ...
          cursor:pointer; color:#94a3b8; font-family:'Outfit',sans-serif; font-size:14px;
          padding:13px 28px; border-radius:12px; transition:all .3s ease;
        }
        ...
        .filter-pill { font-family:'Space Mono',monospace; font-size:11px; text-transform:uppercase; letter-spacing:1.5px; padding:8px 18px; border-radius:999px; cursor:pointer; transition:all .25s ease; border:none; white-space:nowrap; }
        ...
        .filter-scroll { display:flex; gap:10px; overflow-x:auto; -webkit-overflow-scrolling:touch; scrollbar-width:none; padding-bottom:4px; }
        .filter-scroll::-webkit-scrollbar { display:none; }

        @media (max-width:640px) {
          .modal-sheet { position:fixed!important; bottom:0!important; top:auto!important; left:0!important; right:0!important; border-radius:24px 24px 0 0!important; max-height:92vh!important; width:100%!important; max-width:100%!important; padding:32px 20px 40px!important; }
        }
          ...
          <motion.div style={{ position:"relative", zIndex:10, width:"100%", maxWidth:1280, margin:"0 auto", padding: isMobile ? "90px 16px 60px" : isTablet ? "90px 32px 60px" : "100px 48px", y: heroY }}>
            <motion.div initial={{ opacity:0, y:-16 }} animate={{ opacity:1, y:0 }} transition={{ duration:.7 }}>
              <span className="cp-mono" style={{ fontSize: isMobile ? 10 : 11, letterSpacing:"0.2em", color:"#0ea5e9", textTransform:"uppercase", display:"block", marginBottom: isMobile ? 20 : 28 }}>
                — Purple Techno-Vision LLP &nbsp;·&nbsp; Careers
              </span>
            </motion.div>
          ...
          <div style={{ maxWidth:1280, margin:"0 auto", padding:`0 ${px}` }}>
            ...
            <Rev delay={0.05}>
              <div className="filter-scroll" style={{ marginBottom: isMobile ? 28 : 48 }}>
                {DEPTS.map(d => (
                  <button key={d} onClick={() => setFilter(d)} className={`filter-pill ${filter === d ? "active" : "inactive"}`}>{d}</button>
                ))}
              </div>
            </Rev>
            <motion.div layout style={{ display:"grid", gridTemplateColumns: isMobile ? "1fr" : bp === "tablet-sm" ? "repeat(2,1fr)" : "repeat(auto-fill,minmax(320px,1fr))", gap: isMobile ? 16 : 24 }}>
              <AnimatePresence mode="popLayout">
                {filtered.map((job,i) => (
                  <motion.div key={job.id} layout initial={{ opacity:0, scale:.92, y:24 }} animate={{ opacity:1, scale:1, y:0 }} exit={{ opacity:0, scale:.92 }} transition={{ duration:.5, delay:i*0.06, ease:[0.16,1,0.3,1] }}>
        ...
        <AnimatePresence>
          {selectedJob && modalMode && (
            <motion.div
              ...
              <motion.div
                ref={modalRef}
                className="cp-glass-md modal-sheet"
                initial={isMobile ? { y:"100%" } : { scale:0.95, opacity:0 }}
                animate={isMobile ? { y:0 } : { scale:1, opacity:1 }}
                exit={isMobile ? { y:"100%" } : { scale:0.95, opacity:0 }}
                transition={{ type:"spring", damping:30, stiffness:300 }}
                style={{ width:"100%", maxWidth: isMobile ? "100%" : 760, maxHeight: isMobile ? "92svh" : "90vh", overflowY:"auto", borderRadius: isMobile ? "24px 24px 0 0" : 24, padding: isMobile ? "32px 20px 40px" : "44px 48px", position:"relative", border:`1px solid ${selectedJob.color}25` }}
                onClick={e => e.stopPropagation()}>

                {isMobile && <div style={{ width:40, height:4, borderRadius:2, background:"rgba(255,255,255,0.15)", margin:"0 auto 24px" }} />}
                <button onClick={closeModal} style={{ position:"absolute", top: isMobile ? 16 : 20, right: isMobile ? 16 : 20, width:36, height:36, borderRadius:10, background:"rgba(255,255,255,0.06)", border:"1px solid rgba(255,255,255,0.1)", color:"#94a3b8", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", fontSize:18, lineHeight:1, transition:"all .2s" }}>×</button>

                {modalMode === "details" && (
                  <DetailsPanel job={selectedJob} color={selectedJob.color} onApply={() => { setModalMode("apply"); setSuccess(false); setErrors({}); setTouched({}); }} isMobile={isMobile} />
                )}
                {modalMode === "apply" && (
                  <ApplyPanel
                    job={selectedJob} form={form} errors={errors} touched={touched}
                    success={success} onSubmit={handleSubmit} onClose={closeModal}
                    onChange={handleChange} onBlur={handleBlur}
                    submitAttempted={submitAttempted} isMobile={isMobile}
                  />
                )}
              </motion.div>
            </motion.div>
...
const JobCard = ({ job, onApply, onDetails, isMobile }) => (
  <div className="cp-glass job-card" style={{ borderRadius:20, padding: isMobile ? 20 : 28, height:"100%", display:"flex", flexDirection:"column", position:"relative", overflow:"hidden", borderTop:`1px solid ${job.color}30`, borderLeft:"1px solid rgba(255,255,255,0.06)", borderRight:"1px solid rgba(255,255,255,0.06)", borderBottom:"1px solid rgba(255,255,255,0.06)" }}>
    ...
    <div style={{ display:"flex", gap:14, marginBottom:20, flexWrap:"wrap" }}>
      <span style={{ fontSize:12, color:"#64748b", display:"flex", alignItems:"center", gap:5 }}><FiClock size={12}/> {job.experience}</span>
      <span style={{ fontSize:12, color:"#64748b", display:"flex", alignItems:"center", gap:5 }}><FiMapPin size={12}/> {job.location}</span>
    </div>
    <div style={{ display:"flex", gap:8 }}>
      <button className="cp-btn-primary" style={{ flex:1, padding:"11px 0", fontSize:13, borderRadius:10 }} onClick={() => onApply(job, "apply")}>Apply Now</button>
      <button className="cp-btn-ghost"   style={{ flex:1, padding:"11px 0", fontSize:13, borderRadius:10 }} onClick={() => onDetails(job, "details")}>View Details</button>
    </div>
  </div>
);
...
const DetailsPanel = ({ job, color, onApply, isMobile }) => (
  <div>
    ...
    <div style={{ display:"flex", gap:16, marginBottom:24, flexWrap:"wrap" }}>
      <span style={{ fontSize:13, color:"#64748b", display:"flex", alignItems:"center", gap:6 }}><FiClock size={14}/>{job.experience}</span>
      <span style={{ fontSize:13, color:"#64748b", display:"flex", alignItems:"center", gap:6 }}><FiMapPin size={14}/>{job.location}</span>
    </div>
    ...
    {[{ heading:"Key Responsibilities", items:job.responsibilities }, { heading:"Required Skills", items:job.skills }].map(({ heading, items }) => (
      <div key={heading} style={{ marginBottom:24 }}>
        <h4 style={{ fontFamily:"'DM Serif Display',serif", fontSize: isMobile ? 16 : 19, fontWeight:400, color:"#e2e8f0", marginBottom:12, display:"flex", alignItems:"center", gap:10 }}>
          <span style={{ width:4, height:18, borderRadius:2, background:`linear-gradient(${color},transparent)`, display:"inline-block", flexShrink:0 }} />{heading}
        </h4>
...
/* ─────────────────────────────────────────
   APPLY PANEL — WITH FULL VALIDATION
───────────────────────────────────────── */
const ApplyPanel = ({ job, form, errors, touched, success, onSubmit, onClose, onChange, onBlur, submitAttempted, isMobile }) => {
  const [shakeKey, setShakeKey] = useState(0);

  const handleFormSubmit = (e) => {
    e.preventDefault();
    const hasErrors = Object.values(errors).some(Boolean) || !form.name || !form.email || !form.phone || !form.cover;
    if (hasErrors) { setShakeKey(k => k + 1); }
    onSubmit(e);
  };
  ...
  const hasAnyError = submitAttempted && Object.values(errors).some(Boolean);
  ...
  return (
    <div>
      ...
      <h2 style={{ fontFamily:"'DM Serif Display',serif", fontSize: isMobile ? 22 : 28, fontWeight:400, color:"#f8fafc", margin:"0 0 8px", paddingRight:40 }}>{job.title}</h2>
      ...
      <AnimatePresence>
        {hasAnyError && (
          <motion.div
            ...
            style={{ display:"flex", alignItems:"center", gap:10, padding:"12px 16px", borderRadius:10, background:"rgba(248,113,113,0.08)", border:"1px solid rgba(248,113,113,0.3)", marginBottom:16 }}>
            <FiAlertCircle size={16} color="#f87171" style={{ flexShrink:0 }} />
            <p style={{ margin:0, fontSize:13, color:"#f87171", fontFamily:"'Outfit',sans-serif" }}>
      ...
      <form onSubmit={handleFormSubmit} noValidate>
        {/* Name + Email row */}
        <div style={{ display:"grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: isMobile ? 16 : 18, marginBottom: isMobile ? 16 : 18 }}>
          {/* ── NAME — letters only ── */}
          <ValidatedField
            label="Full Name" fieldKey="name"
            value={form.name} placeholder="e.g. Priya Sharma"
            onChange={onChange} touched={touched} errors={errors}
          />
          {/* ── EMAIL ── */}
          <ValidatedField
            label="Email Address" fieldKey="email" type="email"
        ...
        <div style={{ display:"grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: isMobile ? 16 : 18, marginBottom: isMobile ? 16 : 18 }}>
          ...
          <ValidatedField
            label="Mobile Number (10 digits)" fieldKey="phone" type="tel"
            value={form.phone} placeholder="9876543210"
            onChange={onChange} touched={touched} errors={errors}
          />
          {/* Position — read only */}
          <ValidatedField
            ...
            onChange={() => {}} touched={{}} errors={{}} readOnly
          />
        </div>
        ...
        <div style={{ marginBottom: isMobile ? 16 : 18 }}>
          <label style={{
            fontSize:11, color: (touched.cover && errors.cover) ? "#f87171" : (touched.cover && !errors.cover && form.cover.trim().length >= 20) ? "#4ade80" : "#64748b",
            display:"block", marginBottom:8,
            fontFamily:"'Space Mono',monospace", letterSpacing:"0.1em", textTransform:"uppercase", transition:"color 0.2s",
          }}>
            Cover Letter {touched.cover && errors.cover && <span>*</span>}
          </label>
          <div style={{ position:"relative" }}>
            <textarea
              ...
              style={{
                ...
                borderColor: (touched.cover && errors.cover) ? "rgba(248,113,113,0.6)" : (touched.cover && !errors.cover && form.cover.trim().length >= 20) ? "rgba(74,222,128,0.5)" : undefined,
                background: (touched.cover && errors.cover) ? "rgba(248,113,113,0.05)" : (touched.cover && !errors.cover) ? "rgba(74,222,128,0.04)" : undefined,
              }}
            />
          </div>
            ...
            <AnimatePresence>
              {touched.cover && errors.cover && (
                <motion.p initial={{ opacity:0, y:-4, height:0 }} animate={{ opacity:1, y:0, height:"auto" }} exit={{ opacity:0, y:-4, height:0 }}
                  style={{ fontSize:11, color:"#f87171", display:"flex", alignItems:"center", gap:4, margin:0, fontFamily:"'Space Mono',monospace" }}>
                  <FiAlertCircle size={11}/> {errors.cover}
                </motion.p>
              )}
            </AnimatePresence>
            <span style={{ fontSize:10, color: form.cover.trim().length >= 20 ? "#4ade80" : "#475569", marginLeft:"auto", fontFamily:"'Space Mono',monospace" }}>
              {form.cover.trim().length}/20 min
            </span>
          </div>
        </div>
        ...
        <div style={{ marginBottom:28 }}>
          <label style={{ fontSize:11, color:"#64748b", display:"block", marginBottom:8, fontFamily:"'Space Mono',monospace", letterSpacing:"0.1em", textTransform:"uppercase" }}>
            Resume / CV
          </label>
          <label style={{ display:"flex", alignItems:"center", gap:12, padding:"13px 14px", background:"rgba(255,255,255,0.04)", border:"1px dashed rgba(255,255,255,0.12)", borderRadius:12, cursor:"pointer", transition:"all .2s" }}>
            ...
            <span style={{ fontSize:13, color: form.resume ? "#e2e8f0" : "#475569", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>
              {form.resume ? form.resume.name : "Upload PDF or Word document"}
            </span>
            <input type="file" accept=".pdf,.doc,.docx" onChange={e => onChange("resume", e.target.files[0])} style={{ display:"none" }} />
          </label>
        ...
        <div style={{ display:"flex", flexDirection: isMobile ? "column" : "row", gap:10 }}>
          <button type="submit" className="cp-btn-primary" style={{ flex:1, padding:"14px 0", fontSize:14 }}>
            Submit Application
          </button>
          <button type="button" className="cp-btn-ghost" style={{ flex:1, padding:"14px 0", fontSize:14 }} onClick={onClose}>
            Cancel
          </button>
        </div>

        <p style={{ textAlign:"center", fontSize:11, color:"#334155", marginTop:18, fontFamily:"'Space Mono',monospace" }}>
